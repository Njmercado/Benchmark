...
router.get("/", function(req, res){
  ...
  for (var j = 0; j <= iteraciones; j++) {
    ...
    for(var t = 0; t < registros; t++){
      ...
      //sql_adress = sql_adress.concat(` ("${registro.id}", "${adress.city}", "${adress.zip}", "${adress.street}", "${adress.number}"),`)
      //sql_city = sql_city.concat(` ("${registro.id}", "${registro.certificate_number}", "${registro.business_name}", "${registro.date}", "${registro.result}", "${registro.sector}"),`)
    }
